...
public class StudentManagement {
    ...
    public static boolean sameGroup(Student s1, Student s2) {
     ...
     public void list(Student stu) { groupstudent[count]=stu; count++; }

    public void Showlist() {
        for(int i=0;i<count;i++) {
            System.out.println(groupstudent[i].getName()+ " ");
        }
    ...
    void studentsByGroup() {
        ...
        for (int i = 0; i < count; i++) {
            if (A[i] == true) {
                String t = groupstudent[i].getGroup();
                System.out.print("Sinh vien thuoc lop " + t + " bao gom: ");
                for (int j = 0; j < count; j++) {
                    if (groupstudent[j].getGroup().equals(t)) {
                        System.out.print(groupstudent[j].getName()+ " ");
                        A[j] = false;
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        student3.getInfo();
        student4.getInfo();
        System.out.println(sameGroup(student2, student3));
        System.out.println(sameGroup(student2, student4));

       list1.list(student2); list1.list(student3); list1.list(student4); list1.list(studen11);
        list1. Showlist();
       list1.removeStudent("17020889");
       list1. Showlist();
